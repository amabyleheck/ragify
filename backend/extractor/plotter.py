...
def plot_graph(title: str, x_label: str, x_values, data):
    ...
    if len(data) == 0:
        raise Exception("Não foi possível importar nenhum resultado. Você tem certeza que populou o diretório corretamente?")
    ...
    for file in os.scandir(directory):
        ...
        if file.name[length - 5:] == '.xlsx':
            ...
            if isinstance(x_values[0], int):
                ...
                final_results_dict[model]['retriever_accuracy'][int(parameter_value)] = retriever_accuracy
            else:
                final_results_dict[model]['accuracy'][parameter_value] = accuracy
                final_results_dict[model]['retriever_accuracy'][parameter_value] = retriever_accuracy
